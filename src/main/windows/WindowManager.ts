...
export class WindowManager {
  ...
  private attachWebViewReady(
    ...
    if (
      ...
      this.injector.injectNewConversationWatcher(view.webContents).catch(() => {});
    }
  ...
  public async applyDefaultModelMode(wc: WebContents): Promise<void> {
    ...
    if (mode === 'simple') return; // 简单模式是页面默认，无需切换

    const wcId = wc.id;
    const now = Date.now();
    const last = this.lastDefaultModelAt.get(wcId) ?? 0;
    if (now - last < 800) return; // 去抖：同一会话 0.8s 内只切换一次（DeepSeek 同窗跨新对话保留模型，已无可见影响，仅兜底）
    if (this.switchingWc.has(wcId)) return; // 串行：正在切换则跳过，避免并发点击 radio

    this.switchingWc.add(wcId);
    try {
      await this.injector.switchModelMode(wc, mode).catch(() => {});
      // Bug3 修复：setDeepThink 已实现但此前从未被调用。切换默认模型后，按设置同步「深度思考」开关，
      // 使「默认深度思考」设置真正生效（新建对话 / 从托盘唤出主窗口后均会走到这里）。
      if (this.injector) {
        await this.injector.setDeepThink(wc, this.config.get('deepThinkEnabled') === true).catch(() => {});
      }
    } finally {
      this.switchingWc.delete(wcId);
      // 切换「完成」后才记录时间戳：避免把「紧邻的两次真实新建对话」误去抖掉
      // （第二次本就已是默认模型、无副作用，但这样更稳妥，且窗口缩到 0.8s 进一步降低误判概率）。
